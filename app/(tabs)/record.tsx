...
export default function RecordScreen() {
  ...
  const timerRef = useRef<NodeJS.Timeout | null>(null); // Ref to store the timer interval
  ...
  }, [isPlaying, connectedDevice]);

  const averageHeartRate =
    heartRateCount > 0 ? (totalHeartRate / heartRateCount).toFixed(1) : "N/A";
  ...
  const stopSimulation = async () => {
    ...
    if (!connectedDevice) return;
      ...
      console.log("INSIDE");
      connectToDeviceWorkout(connectedDevice); //this is to get the final workout NOT to just connect to device
    }
  };
  ...
  const handleRecordingUpload = async (
    ...
    try {
      await uploadRecording(averageHeartRate, exercise, reps, elapsedTime);
    } catch (error) {
  ...
  const handleExerciseData = async (data: string) => {
    try {
      ...
      await handleRecordingUpload(exercise, reps, elapsedTime);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.contentContainer}>
        ...
        <LineChart
          data={{
            labels: Array.from({ length: dataPoints.length }, (_, i) =>
              (i + 1).toString()
            ),
